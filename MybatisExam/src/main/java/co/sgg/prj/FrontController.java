...
import co.sgg.prj.member.command.MemberDelete;
import co.sgg.prj.member.command.MemberEdit;
import co.sgg.prj.member.command.MemberIdCheck;
import co.sgg.prj.member.command.MemberInsert;
...
//       System.out.println(request.getRemoteUser());
		...
		if (!viewPage.endsWith(".do")) {
			if (viewPage.startsWith("ajax:")) {
				response.setContentType("text/html; charset=UTF-8"); // 돌려줄 페이지의 컨텐츠 타입
				response.getWriter().append(viewPage.substring(5)); // ajax: 부분 지우고 뒤에 숫자만 출력
				return;
			...
			response.sendRedirect(viewPage); // command return 값이 *.do일때 처리
		}
